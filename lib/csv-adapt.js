...
// The category set the classifier maps every merchant into. Kept deliberately
// small and bank-agnostic so groupings stay meaningful across statements.
const CATEGORIES = [
  'Income',
  'Transfers',
  'Groceries',
...
const PROFILE_TOOL = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      bankName: {
        type: 'string',
        description: 'Bank or account name if identifiable from the file, else "Unknown".',
      },
      currencyCode: { type: 'string', description: 'ISO currency code, e.g. GBP, USD.' },
      dateColumn: { type: 'string', description: 'Exact header text of the transaction date column.' },
      dateFormat: {
        type: 'string',
      ...
      descriptionColumn: {
        type: 'string',
        description: 'Exact header text of the transaction description/narrative column.',
      },
      amountConvention: {
        type: 'string',
      ...
      amountColumn: {
        type: 'string',
        description: 'For "signed": exact header text of the single amount column.',
      },
      moneyInColumn: {
        type: 'string',
        description: 'For "split"/"debit_credit": exact header text of the money-in / credit column.',
      },
      moneyOutColumn: {
        type: 'string',
        description: 'For "split"/"debit_credit": exact header text of the money-out / debit column.',
      },
    },
    required: ['headerRowIndex', 'dateColumn', 'dateFormat', 'descriptionColumn', 'amountConvention'],
  },
};
...
function applyProfile(rawText, profile, bankName) {
  ...
  const fromHeader = lines.slice(profile.headerRowIndex).join('\n');
...
const CLASSIFY_TOOL = {
  ...
  description: 'Assign each merchant/payee string to exactly one spending category.',
  input_schema: {
    type: 'object',
    properties: {
      assignments: {
        ...
        items: {
          type: 'object',
          properties: {
            merchant: { type: 'string', description: 'The merchant string, copied verbatim.' },
...
async function classifyMerchants(keys, { apiKey } = {}) {
    ...
    messages: [
      {
        role: 'user',
        content:
          `Classify each of these bank-statement merchant/payee strings into one category from this set: ${CATEGORIES.join(', ')}. ` +
          `Incoming salary/wages -> Income; person-to-person or own-account movements -> Transfers; bank fees/interest -> Fees & Charges. ` +
          `Return one assignment per input, copying the merchant verbatim.\n\n` +
          keys.map((k) => `- ${k}`).join('\n'),
      },
    ],
  });
  ...
  for (const a of assignments) {
    if (a && typeof a.merchant === 'string') {
      map[a.merchant] = CATEGORIES.includes(a.category) ? a.category : 'Other';
    }
  }
  return map;
}
...
function toCanonicalCsv(rows) {
  ...
  const body = rows
    .map((r) => [r.Date, r.Description, r.Amount, r.Category, r.Bank].map(csvEscape).join(','))
    .join('\n');
  return `${header}\n${body}\n`;
}
...
function looksCanonical(rawText) {
  ...
  return first.includes('date') && first.includes('amount') && first.includes('description');
}
...
async function transformAndCategorise(rawText, profile, { apiKey, onEvent } = {}) {
  const emit = onEvent || (() => {});
  ...
  await emit({ stage: 'transform', message: `Normalised ${rows.length} transactions.` });
  ...
  await emit({ stage: 'categorise', message: `Classifying ${uniqueKeys.size} unique merchants…` });
  const categoryByKey = await classifyMerchants([...uniqueKeys], { apiKey });
  for (const row of rows) {
    row.Category = categoryByKey[keyByDescription.get(row.Description)] || 'Other';
  }
  ...
  return { csv: toCanonicalCsv(rows), profile, rowCount: rows.length, bankName, skipped: false };
}
...
async function adaptCsv(rawText, { apiKey, onEvent } = {}) {
  ...
  if (looksCanonical(rawText)) {
    await emit({ stage: 'skip', message: 'CSV is already in the canonical format — using it as-is.' });
    return { csv: rawText, profile: null, rowCount: null, bankName: null, skipped: true };
  }
