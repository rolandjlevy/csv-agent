...
function normaliseDate(value, format) {
  ...
  // day-first — real statements sometimes switch format mid-export (e.g. an
  // amended transaction row). Detected per-value rather than trusting the
  // file-wide profile blindly, so a handful of outlier rows don't land their
  // digits in the wrong day/month/year slot.
  const iso = text.match(/^(\d{4})-(\d{1,2})-(\d{1,2})/);
  if (iso) return `${pad(iso[3])}/${pad(iso[2])}/${iso[1]}`;
  if (format === 'YYYY-MM-DD') return text; // declared ISO but value didn't match it
...
// Splits keys into ones already present (and still valid) in a saved
// merchant map, and ones that need a fresh LLM call. A saved entry whose
// category no longer exists in ACCOUNTS (e.g. the chart of accounts changed)
// is treated as unknown rather than trusted blindly.
function classifyMerchantsWithCache(keys, knownMap = {}) {
  const known = {};
  const unknown = [];
  for (const key of keys) {
    if (
      Object.prototype.hasOwnProperty.call(knownMap, key) &&
      ACCOUNT_NAMES.includes(knownMap[key])
    ) {
      known[key] = knownMap[key];
    } else {
      unknown.push(key);
    }
  }
...
async function classifyMerchants(keys, { apiKey } = {}) {
  ...
  for (const a of assignments) {
    if (a && typeof a.merchant === 'string') {
      map[a.merchant] = ACCOUNT_NAMES.includes(a.category)
        ? a.category
        : 'Uncategorised';
    }
  }
...
// A file is already canonical (skip adaptation, no LLM cost) only if its
// header row has actual columns named exactly date/amount/description — not
// merely containing those words as a substring. A naive substring check
// false-positives on extremely common real bank headers like "Transaction
// Date", "Transaction Description", "Debit Amount" — which would silently
// skip adaptation on a file that badly needs it (raw bank columns are passed
// straight through, formats unparsed, nothing categorised).
...
async function transformAndCategorise(
  rawText,
  profile,
  { apiKey, onEvent, knownMerchantMap } = {},
) {
  const emit = onEvent || (() => {});
  const bankName = profile.bankName || 'Unknown';
  const knownMap = knownMerchantMap || {};
  ...
  const { known, unknown } = classifyMerchantsWithCache(
    [...uniqueKeys],
    knownMap,
  );
  await emit({
    ...
    message:
      Object.keys(known).length > 0
        ? `Classifying ${unknown.length} new merchants (${Object.keys(known).length} reused from saved profile)…`
        : `Classifying ${uniqueKeys.size} unique merchants…`,
  });
  const newlyClassified = await classifyMerchants(unknown, { apiKey });
  const categoryByKey = { ...known, ...newlyClassified };
  for (const row of rows) {
    row.Category =
  ...
  return {
    ...
    bankName,
    skipped: false,
    merchantMap: { ...knownMap, ...newlyClassified },
    newMerchantKeys: unknown,
  };
}
...
async function adaptCsv(rawText, { apiKey, onEvent, knownMerchantMap, resolveProfile } = {}) {
  const emit = onEvent || (() => {});
  ...
  const detected = await detectProfile(rawText, { apiKey });
  await emit({
    stage: 'profile',
    message: `Detected ${detected.bankName || 'Unknown'} export — ${detected.amountConvention} amounts, header on row ${detected.headerRowIndex}.`,
    profile: detected,
  });

  let profile = detected;
  let mergedKnownMap = knownMerchantMap;
  if (resolveProfile) {
    const resolved = await resolveProfile(detected);
    if (resolved) {
      profile = resolved.profile;
      mergedKnownMap = { ...(knownMerchantMap || {}), ...(resolved.merchantMap || {}) };
      await emit({
        stage: 'profile',
        message: `Matched saved recipe "${resolved.name}" — reusing its column mapping.`,
        profile,
      });
    }
  }

  return transformAndCategorise(rawText, profile, {
    apiKey,
    onEvent,
    knownMerchantMap: mergedKnownMap,
  });
}
